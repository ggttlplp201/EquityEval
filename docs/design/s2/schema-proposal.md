...
| N1 topics / profile revisions / relevance relationships | canonical subject identity and aliases; category and official namespace/identifier (including bill jurisdiction/session); issuer/business-driver connection, supporting evidence or explicit hypothesis; effective dates and knowledge/retrieval times; membership generation, user topic controls/persistent exclusions and coverage gaps. Many-to-many topic/issuer links, versioned rather than ticker-keyword lists. |
| N1 `news_items` / revisions / development groups / claims | publication/event/retrieval times and precision; URL, permitted content, capture/hash; immutable source claims, exact evidence locators and claim status; underlying development identity, syndication origin, conflicting accounts and cited relevance. Source text is untrusted data; a source's announcement is not proof of its commercial claims. |
| N1 driver series / observation revisions | topic/issuer, metric definition, unit, measurement interval, scope and methodology, source capture/vintage, value or explicit gap and quality flags. Keep token circulation, transfer activity and issuer revenue distinct. Reviewed typed references to financial facts prevent news numbers from silently replacing normalized facts. |
| N1 `event_briefs` / impact assessments / evidence links | immutable security, development/topic/relevance/profile revisions, exact claim/observation input references and cutoffs, analysis/model/prompt versions, evidence and counterevidence; driver-level direction, horizon, confidence rationale, gaps, conditional valuation links and falsification/next-review conditions. Parent assessment and change reason distinguish new evidence, corrections and model-only regeneration. No autonomous assumption/valuation changes. |
| N1 `notification_preferences`, `alerts`, `deliveries`, `delivery_attempts` | recipient/channel preferences separate from prompts; immutable alert identity with all included stock-assessment revisions; membership generation, due time and state. Shared uniqueness on recipient/channel/development/material revision/purpose prevents separate macro/topic producers from double-sending; purpose distinguishes configured lead times, release, correction and assessment changes. Stable provider idempotency key; recheck membership, topic controls and current relevance before send, suppressing obsolete components. Uncertain provider responses require reconciliation, not blind duplicate sends. |
...
N1 stock-topic details and the CRCL seed example are linked from the
[feature requirements](../../features/N1-stock-topic-monitoring.md). These remain
later proposed contracts, not additional S2a/S2b migrations or an approved API.
Shared macro/topic development identity and assessment revisions must support
deduplication; initial historical coverage is a baseline, not fresh breaking news.
