...
export function PlotAxes({ xAxis, yAxis }: { xAxis: Axis; yAxis: Axis }) {
  return <g className="plot-axes" aria-hidden="true">
    ...
    {xAxis.ticks.map((tick) => <text key={`${tick.label}-${tick.position}`} x={tick.position} y="270" textAnchor="middle">{tick.label}</text>)}
  </g>;
}
export function HistoryChart({ data, selected, sectors, onDetail, describeDetail }: { data: HistoryPlot; selected: string[]; sectors: Sector[]; onDetail: (id: string) => void; describeDetail: (id: string) => string }) {
  const id = useId();
  const series = data.series.filter((item) => selected.includes(item.sectorId));
  return <>
    <svg className="analytical-plot" viewBox="0 0 760 285" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
...
export function DistributionChart({ data, activeBin, pinnedCompany, onBin, onDetail, describeDetail }: { data: Distribution; activeBin: string | null; pinnedCompany: string | null; onBin: (id: string | null) => void; onDetail: (id: string) => void; describeDetail: (id: string) => string }) {
  ...
  return <>
    {data.emptyReason ? <div className="empty-chart">{data.emptyReason}</div> : <svg className="analytical-plot" viewBox="0 0 760 285" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
...
export function ScatterChart({ data, sectors, sized, selected, onSelect, onDetail, describeDetail }: { data: ScatterPlot; sectors: Sector[]; sized: boolean; selected: string; onSelect: (id: string) => void; onDetail: (id: string) => void; describeDetail: (id: string) => string }) {
  const id = useId();
  return <>
    <svg className="analytical-plot scatter-plot" viewBox="0 0 760 285" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
      ...
      <PlotAxes xAxis={data.xAxis} yAxis={data.yAxis} />
      {data.points.map((point) => <g key={point.sectorId} transform={`translate(${point.x}, ${point.y})`}><circle r={sized ? point.radius : 6} fill={lookup(sectors, point.sectorId).color} fillOpacity=".8" stroke={selected === point.sectorId ? "var(--ink)" : "white"} strokeWidth="2" tabIndex={0} role="button" aria-label={`${lookup(sectors, point.sectorId).label}: P/E ${point.xLabel}, growth ${point.yLabel}. Select sector.`} onClick={() => onSelect(point.sectorId)} onKeyDown={(event) => activate(event, () => onSelect(point.sectorId))}><title>{`${lookup(sectors, point.sectorId).label} · P/E ${point.xLabel} · Growth ${point.yLabel}\n${describeDetail(point.detailId)}\n${describeDetail(point.growthDetailId)}`}</title></circle><text x="11" y="-9" className="scatter-label">{lookup(sectors, point.sectorId).label}</text></g>)}
    </svg>
