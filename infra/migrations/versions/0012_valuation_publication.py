...
BEGIN
 ...
 IF s ? 'anyOf' THEN
 FOR v IN SELECT value FROM jsonb_array_elements(s->'anyOf') LOOP
...
BEGIN
 ...
 END IF;
 END LOOP;
 IF jsonb_array_length(p->'judgments')<>13 OR
 (SELECT array_agg(x->>'parameter' ORDER BY x->>'parameter') FROM jsonb_array_elements(p->'judgments') x)
...
CREATE TABLE valuation_assumption_entries (
 assumption_id UUID NOT NULL REFERENCES valuation_assumption_sets(id), parameter_key TEXT NOT NULL,
 unit TEXT NOT NULL, effective_from DATE NOT NULL, effective_to DATE NOT NULL,
 entry_text TEXT NOT NULL, PRIMARY KEY(assumption_id,parameter_key),
...
BEGIN
 ...
 INSERT INTO valuation_assumption_entries SELECT new_id,j->>'parameter',j->>'unit',
...
def upgrade():
    ...
    op.execute(
        "GRANT EXECUTE ON FUNCTION valuation_assumptions_valid(jsonb),valuation_shape_valid(jsonb,jsonb,jsonb,integer) TO equity_runtime"
    )
